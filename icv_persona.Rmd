...
c.pers<-read.csv("Pichincha_CSV_Poblac.csv")
c.hogar<-read.csv("Pichincha_CSV_Hogar.csv")
c.viv<-read.csv("Pichincha_CSV_Vivienda.csv")

### Azuay
c.pers<-read.csv("Azuay_CSV_Poblac.csv")
c.hogar<-read.csv("Azuay_CSV_Hogar.csv")
c.viv<-read.csv("Azuay_CSV_Vivienda.csv")
...
c.pers$codhog<-paste(c.pers$I01,c.pers$I02,c.pers$I03,c.pers$I04,c.pers$I05,c.pers$I06,c.pers$I09,c.pers$I10,sep = "")
...
c.pers<-c.pers %>% left_join(seg.jefe, by = "codhog")
...
c.pers$codviv<-paste(c.pers$I01,c.pers$I02,c.pers$I03,
                     c.pers$I04,c.pers$I05,c.pers$I06,c.pers$I09,sep = "")


#med.segsoc <- ddply(c.pers,.(codviv),summarize, med.segsoc=mean(salud, na.rm=TRUE))
...
c.pers <- c.pers %>% 
  mutate(anos_educ = case_when(P03 == 8 ~ 2,
                               P03 == 9 ~ 3,
                               P03 == 10 ~ 4,
                               P03 == 11 ~ 5,
                               P03 == 12 ~ 6,
...
c.pers <- c.pers %>% 
  mutate(educacion = case_when(P03 > 6  & GRAESC != 99 ~ (GRAESC/anos_educ) ))
...
#med.educ <- ddply(c.pers,.(codviv),summarize, med.educ=mean(educacion_recode))
...
c.viv$codviv<-paste(c.viv$I01,c.viv$I02,c.viv$I03,c.viv$I04,c.viv$I05,c.viv$I06,c.viv$I09,sep = "")

c.viv<-c.viv %>% left_join(med.segsoc, by = "codviv")
c.viv<-c.viv %>% left_join(med.educ, by = "codviv")
...
c.hogar$codviv<-paste(c.hogar$I01,c.hogar$I02,c.hogar$I03,c.hogar$I04,c.hogar$I05,c.hogar$I06,c.hogar$I09,sep = "")

#med.comunicacion <- ddply(c.hogar,.(codviv),summarize, med.comunicacion=mean(comunicacion))
...
#med.combu <- ddply(c.hogar,.(codviv),summarize, med.combu=mean(combu))
...
c.hogar <- c.hogar %>% 
  mutate(hacina =  case_when(TP1 == 1 ~ 1))
...
c.hogar<- c.hogar %>% 
  mutate(hacinarecod =  case_when(hacinadormitorio > 1 ~ ((1+(hacinadormitorio-1)/(32-1))), 
                                  hacinadormitorio <=1 ~ hacinadormitorio ))

  # *4. OTRAS VARIABLES PARA EL CÁLCULO DE HACINAMIENTO.
  # *4.1 Calcular la existencia de cocina exclusiva para el hogar.

c.hogar <- c.hogar %>% 
  mutate(cocina =  case_when(H02 == 1 ~ 1,
                             H02 == 2 ~ 0))

 # *4.2 Calcular la existencia de baño exclusivo.

c.hogar <- c.hogar %>% 
    mutate(bano =  case_when(H03 == 1 ~ 1,
                             H02 >= 2 & H02 <= 3 ~ 0))
...
#cuarto.extra <- ddply(c.hogar,.(codviv),summarize, cuartos=sum(H01))

cuarto.extra <- c.hogar %>%
group_by(codviv) %>%
dplyr::summarize(Cuartos = sum(H01, na.rm=TRUE))

c.hogar<-c.hogar %>% left_join(cuarto.extra, by = "codviv")
...
c.viv$codviv<-paste(c.viv$I01,c.viv$I02,c.viv$I03,c.viv$I04,c.viv$I05,c.viv$I06,c.viv$I09,sep = "")
col14<-c.viv[ -c(1:26,28:32) ]
c.hogar<- c.hogar %>% left_join(col14, by = "codviv")

c.hogar$cuarto.extra <- c.hogar$V14-c.hogar$Cuartos
...
c.hogar$hacinamientofinal <- (c.hogar$hacinarecod+c.hogar$cocina+c.hogar$bano+c.hogar$cuarto.extraf)/4
...
#5 las variables de hacinamiento, combustible y comunicacion en la base de vivienda

c.viv<- c.viv %>% left_join(med.haci, by = "codviv")
c.viv<- c.viv %>% left_join(med.combu, by = "codviv")
c.viv<- c.viv %>% left_join(med.comunicacion, by = "codviv")
...
c.viv$SERVICIOS<- (c.viv$AGUA_SAN*0.5)+(c.viv$ENERGIA*0.25)+(c.viv$Mean_comunic*0.25)
...
##1 Crear columna de parroquia y sector para la segmentación

c.viv$codcanton<-paste(c.viv$I01,c.viv$I02,c.viv$I03,sep = "")

#quito
df.uio<-c.viv %>%
  dplyr::filter(codcanton %in% c(170150, 170155,170157,170184))
...
df.cue<-c.viv %>%
  dplyr::filter(codcanton %in% c("010150"))


df.cue<-df.cue %>%  
        mutate(quantile_rank = ntile(df.cue$ICV,4))

df.cue<-df.cue %>%  
        select(-(9:29),-(31:32),-(34:55))

df.cue$DPA_SECTOR<-paste(df.cue$I01,df.cue$I02,df.cue$I03,df.cue$I04,df.cue$I05,sep = "")

write.csv(df.cue,"CUENCA_ICV_VIVIENDA_COMPLETO.csv")
